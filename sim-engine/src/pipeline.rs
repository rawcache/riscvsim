//! Classic 5-stage RV32IM pipeline (Patterson & Hennessy style).
//!
...
use crate::assembler::{abi_name, Op, Program};
use crate::{DATA_BASE, DATA_SIZE, STACK_LIMIT, STACK_TOP, TEXT_BASE};
use serde::Serialize;
...
struct MemWb {
    ...
    mem_write: Option<(u32, u32, u32)>, // (addr, before, after) word-ish view
}
...
struct Undo {
    ...
    stats: Stats,
    cycle: u64,
    reg_changes: Vec<(u8, u32)>,     // (reg, old value)
    mem_changes: Vec<(u32, Option<u8>)>, // (addr, old byte; None = was unset)
    log_added: usize,
    last_view: CycleView,
...
impl Pipeline {
    ...
    pub fn reset(&mut self) {
        ...
        for (i, b) in self.program.data.iter().enumerate() {
            if *b != 0 {
                self.mem.insert(DATA_BASE + i as u32, *b);
            }
        }
        self.pc = TEXT_BASE;
    ...
    fn mem_region_ok(addr: u32, len: u32) -> bool {
        let end = match addr.checked_add(len - 1) {
            Some(e) => e,
            None => return false,
        };
        (addr >= DATA_BASE && end < DATA_BASE + DATA_SIZE)
            || (addr >= STACK_LIMIT && end <= STACK_TOP + 3)
    }

    fn read_mem(&self, addr: u32, len: u32, line: usize) -> Result<u32, HaltInfo> {
        // Misaligned accesses trap (a valid RV32I implementation choice, and
        ...
        let mut v: u32 = 0;
        for i in 0..len {
            v |= (self.mem.get(&(addr + i)).copied().unwrap_or(0) as u32) << (8 * i);
        }
        Ok(v)
    }
    ...
    fn write_mem(
        ...
        value: u32,
        line: usize,
        mem_changes: &mut Vec<(u32, Option<u8>)>,
    ) -> Result<(u32, u32), HaltInfo> {
        if addr % len != 0 {
            return Err(HaltInfo {
                kind: "error",
        ...
        for i in 0..len {
            before |= (self.mem.get(&(addr + i)).copied().unwrap_or(0) as u32) << (8 * i);
        }
        for i in 0..len {
            let a = addr + i;
        ...
        Ok((before, value & (u32::MAX >> (32 - 8 * len.min(4)))))
    }

    // -- ALU ---------------------------------------------------------------

    fn alu(op: Op, a: u32, b: u32, imm: i32, pc: u32) -> u32 {
        let ia = a as i32;
        match op {
            Op::Add => a.wrapping_add(b),
            Op::Sub => a.wrapping_sub(b),
            Op::Sll => a.wrapping_shl(b & 31),
            Op::Slt => ((ia) < (b as i32)) as u32,
            Op::Sltu => (a < b) as u32,
            Op::Xor => a ^ b,
            Op::Srl => a.wrapping_shr(b & 31),
            Op::Sra => (ia >> (b & 31)) as u32,
            Op::Or => a | b,
            Op::And => a & b,
            Op::Mul => a.wrapping_mul(b),
            Op::Mulh => (((ia as i64) * (b as i32 as i64)) >> 32) as u32,
            Op::Mulhsu => (((ia as i64) * (b as u64 as i64)) >> 32) as u32,
            Op::Mulhu => (((a as u64) * (b as u64)) >> 32) as u32,
            // RV32M division by zero does NOT trap: the spec defines the
            ...
            Op::Slti => ((ia) < imm) as u32,
            Op::Sltiu => (a < imm as u32) as u32,
            Op::Xori => a ^ imm as u32,
            Op::Ori => a | imm as u32,
            Op::Andi => a & imm as u32,
            Op::Slli => a.wrapping_shl((imm as u32) & 31),
            Op::Srli => a.wrapping_shr((imm as u32) & 31),
            Op::Srai => (ia >> ((imm as u32) & 31)) as u32,
            Op::Lui => (imm as u32) << 12,
            Op::Auipc => pc.wrapping_add((imm as u32) << 12),
            // loads/stores: effective address
            _ if op.is_load() || op.is_store() => a.wrapping_add(imm as u32),
    ...
    fn exit_in_flight(&self) -> bool {
        let is_exit = |op: Op| matches!(op, Op::Ecall | Op::Ebreak);
        self.if_id
            .inst()
    ...
    pub fn step_cycle(&mut self) {
        ...
        match &old_mem_wb {
            Slot::Inst(wb) => {
                ...
                if wb.op.writes_rd() && wb.rd != 0 {
                    ...
                    delta = format!("  mem[0x{addr:08x}]: {} -> {}", before as i32, after as i32);
                }
            ...
            Slot::Inst(id) => {
                ...
                if id.op.is_branch() || id.op.is_jump() {
                    ...
                    let (actual_taken, actual_target) = match id.op {
                        Op::Jal => (true, id.pc.wrapping_add(id.imm as u32)),
                        Op::Jalr => (true, a.wrapping_add(id.imm as u32) & !1u32),
                        _ => (
                        ...
                        if prev.op.is_load() && prev.rd != 0 {
                            ...
                            if needs {
                                ...
                                let reason = format!(
                                    ...
                                    prev.rd,
                                    self.program.instrs[prev.index].text
                                );
        ...
        if let Some(target) = flush_to {
            ...
            if let Some(instr) = self.program.instr_at(self.pc) {
                let index = ((self.pc - TEXT_BASE) / 4) as usize;
                stages[0] = StageView {
                    name: "IF",
                    state: StageState::Normal,
                    text: Some(instr.text.clone()),
                ...
                let predicted_taken =
                    entry.valid && entry.tag_pc == self.pc && entry.counter >= 2;
                let predicted_target = if predicted_taken { entry.target } else { 0 };
                if predicted_taken {
                    stages[0].detail = Some(format!(
                        "predicted taken -> 0x{predicted_target:08x}"
                    ));
                }
    ...
    pub fn log_since(&self, since: u64) -> Vec<LogLine> {
        self.log.iter().filter(|l| l.seq >= since).cloned().collect()
    }
...
mod tests {
    ...
    fn forwarding_stress_no_stalls() {
        ...
        assert!(p.stats.cycles <= 8, "unexpected cycle count {}", p.stats.cycles);
    }
...
addi x4, x0, 1\nla x2, value\nlw x1, 0(x2)\nadd x3, x1, x4\n";
        ...
        assert_eq!(p.regs[1], 41);
        assert_eq!(p.regs[3], 42);
...
la x2, value\nlw x1, 0(x2)\naddi x5, x0, 100\nadd x3, x1, x1\n";
        let p = run_program(src);
        assert_eq!(p.regs[3], 14);
        assert_eq!(p.regs[5], 100);
        assert_eq!(p.stats.stalls, 0, "one-instruction gap is covered by MEM/WB forwarding");
    }
...
addi x3, x0, 99\n";
        ...
        assert!(p.stats.mispredictions >= 1, "cold predictor must miss at least once");
...
skip: addi x8, x0, 8\n";
        ...
        assert_eq!(p.stats.flushes, 2, "unconditional-taken beq squashes IF and ID");
    }
...
loop: addi x1, x1, -1\nbne x1, x0, loop\n";
        ...
        assert_eq!(p.stats.mispredictions, 2, "2-bit predictor: only first + last miss");
    }
...
la x2, buf\naddi x3, x0, 123\nsw x3, 4(x2)\nlw x4, 4(x2)\naddi x5, x4, 1\n";
        let p = run_program(src);
        assert_eq!(p.regs[4], 123);
        assert_eq!(p.regs[5], 124);
    }
...
div x3, x1, x2\ndivu x4, x1, x2\nrem x5, x1, x2\nremu x6, x1, x2\n";
        let p = run_program(src);
        assert_eq!(p.regs[3], u32::MAX); // div by zero => -1
        assert_eq!(p.regs[4], u32::MAX);
...
li x1, -2147483648\naddi x2, x0, -1\ndiv x3, x1, x2\nrem x4, x1, x2\n";
        let p = run_program(src);
        assert_eq!(p.regs[3], i32::MIN as u32);
...
ret\n";
    ...
    fn step_back_restores_state() {
        ...
        assert_eq!(p.regs[2], STACK_TOP, "sp must return to its initial value");
...
lw t1, 4(sp)\naddi sp, sp, 8\n";
        let p = run_program(src);
        assert_eq!(p.regs[6], 77);
        assert_eq!(p.regs[2], STACK_TOP);
    }
